...
namespace fidelizPlus_back.Services
{
		...
		where TAccountDTO : AccountDTO, new()
	{
		...
		public void CheckCredentials(int id)
		{
			...
			{
				throw new Break("Unauthorized operation", BreakCode.Unauthorized, 400);
			}
		}
		...
		public TEntity FindEntityByConnectionId(string connectionId)
		{
			...
			return entity != null
				? entity
				: new Break($"{typeof(TEntity).Name} not found", BreakCode.NotFound, 404).Throw<TEntity>();
		}
		...
		public (TPrivate, int) CheckSave(TPrivate dto)
		{
			...
			{
				throw new Break($"'{dto.ConnectionId}' is already used as connectionId", BreakCode.Existing, 400);
			}
